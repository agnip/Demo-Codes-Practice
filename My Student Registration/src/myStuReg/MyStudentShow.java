...
//This Class is responsible for showing Records of all the Students
public class MyStudentShow extends JFrame
{
	...
	public MyStudentShow()
	{
		...
		try(ObjectInputStream oin = new ObjectInputStream(new FileInputStream("Registration.txt")))
		{
			...
			if(arraylist.size() > 0)
			{
				JOptionPane.showMessageDialog(null,"Showing All Records....");
			}
			else
			{
			...
			for(i=0;i<arraylist.size();i++)
			{
				...
				tabbody[i][0] = "                   " + MSI.getRoll() + "";
				tabbody[i][1] =  MSI.getName() + "";
				tabbody[i][2] = "                  " + MSI.getMarks1() + "";
				tabbody[i][3] = "                  " + MSI.getMarks2() + "";
			}
			...
			JLabel JL1 = new JLabel("                      ALL STUDENT RECORDS");
			c.add(JL1);
			JL1.setFont(F1);
